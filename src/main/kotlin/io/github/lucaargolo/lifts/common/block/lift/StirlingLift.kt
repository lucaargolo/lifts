...
class StirlingLift(settings: Settings, platformSpeed: Double, platformRange: Int): Lift(settings, platformSpeed, platformRange) {
    ...
    override fun onUse(state: BlockState, world: World, pos: BlockPos, player: PlayerEntity, hand: Hand, hit: BlockHitResult): ActionResult {
        player.openHandledScreen(object: ExtendedScreenHandlerFactory {
            override fun getDisplayName() = TranslatableText("screen.stirling_lift")

            override fun createMenu(syncId: Int, inv: PlayerInventory, player: PlayerEntity): ScreenHandler {
                return StirlingLiftScreenHandler(syncId, inv, world.getBlockEntity(pos) as StirlingLiftBlockEntity, ScreenHandlerContext.create(world, pos))
            }
